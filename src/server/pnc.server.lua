...
local portal = Instance.new("SpawnLocation",script.ps)
...
portal.Position = Vector3.new((size.Value / 2) * ps.Value,10,(size.Value / 2 * ps.Value))
...
local nheight = math.noise((x + seed.Value) / res.Value * amp.Value,(z + seed.Value) / res.Value * amp.Value)
nheight = math.clamp(nheight,-.5,.5) + .5
return nheight
end
goresiz1:Play()
task.wait(1)
...
p.Position = Vector3.new(x * ps.Value,h * amp.Value * freq.Value,z * ps.Value)
box.Text = [[
X: = ]]..x..[[
Z: = ]]..z..[[
Position = ]]..p.Position.X..[[, ]]..p.Position.Y..[[, ]]..p.Position.Z
local post = {Position = Vector3.new(x * ps.Value,h * amp.Value * freq.Value,z * ps.Value)}p.Position = Vector3.new(portal.Position.X,portal.Position.Y - ps.Value + .001,portal.Position.Z)

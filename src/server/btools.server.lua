...
local tool = lib.Create("Tool",owner.Backpack,true){
	Name = "Building Tools",
	ToolTip = "A tool for building",
	lib.Create("Part",nil,true){
		Name = "Handle",
		Material = "SmoothPlastic",
		Size = Vector3.new(1,1,1)
	}
}
local remote = lib.Create("RemoteFunction",owner.PlayerGui,true){
	Name = "SyncRemote_"..HttpService:GenerateGUID()
}
remote.OnServerInvoke = function(Player,Data)
	if Player == owner then
		local DataType = Data.SyncType
		if DataType == "SyncLoaded" then
			return tool
...
local function upEvent()
	Storage.Target.Anchored = Storage.Anchored
	sync()
end
